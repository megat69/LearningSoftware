...
while mode < 1 or mode > 8:
    mode = int(input(f"{bcolors.WARNING}Choose the mode :{bcolors.ENDC}\n"
        f"{bcolors.WARNING}1) Ordered, one single column to fill ;{bcolors.ENDC}\n"
        f"{bcolors.WARNING}2) Ordered, three columns to fill ;{bcolors.ENDC}\n"
        f"{bcolors.WARNING}3) Random order, three columns to fill ;{bcolors.ENDC}\n"
        f"{bcolors.WARNING}4) INFINITE MODE, THREE COLUMNS, ordered ;{bcolors.ENDC}\n"
        f"{bcolors.WARNING}5) INFINITE MODE, THREE COLUMNS, UNORDERED ;{bcolors.ENDC}\n"
        f"{bcolors.WARNING}6) Choose the lines to work on, 3 columns, ordered ;{bcolors.ENDC}\n"
        f"{bcolors.WARNING}7) Choose the lines to work on, 3 columns, UNORDERED ;{bcolors.ENDC}\n"
        f"{bcolors.WARNING}8) INFINITE MODE, THREE COLUMNS, choose the lines to work on ; UNORDERED{bcolors.ENDC}\n"", 3 colonnes, DANS LE DÉSORDRE{bcolors.ENDC}\n"))
...
if mode == 1:
    ...
    for i in range(len(verbs_list)):
        ...
        answer = input(f"{bcolors.OKBLUE}Conjugate the verb {bcolors.HEADER}{others}{bcolors.OKBLUE}for the time '{column_order[column_selected]}'.{bcolors.ENDC}\n")
        ...
        if isinstance(verbs_list[i][column_selected], list):
            if multi_answers_require_all is True:
                ...
                if answer.lower() in verbs_list[i][column_selected]:  # If the answer is in the correct answers
                    ...
                    print(f"{bcolors.OKGREEN}Congratulations, you're right !{bcolors.ENDC}")
                    ...
                    print(
                        f"{bcolors.FAIL}No, that was '{bcolors.HEADER}{verbs_list[i][column_selected]}{bcolors.FAIL}' !{bcolors.ENDC}")
                    # Adding the wrong answer to the list of total wrong answers
                    wrong_answers.append(f"{verbs_list[i][column_selected]} for the time '{column_order[column_selected]}'")
        if multi_answers_require_all is True:
            if answer.lower() == verbs_list[i][column_selected]:  # If the answer is correct
                ...
                # Print a message with the answer
                print(f"{bcolors.FAIL}No, that was '{bcolors.HEADER}{verbs_list[i][column_selected]}{bcolors.FAIL}' !{bcolors.ENDC}")
                # Adding the wrong answer to the list of total wrong answers
                wrong_answers.append(f"{verbs_list[i][column_selected]} for the time '{column_order[column_selected]}'")

        if answer_time < 5:
            print(f"{bcolors.OKBLUE}{answer_time}{bcolors.OKBLUE} seconds to answer ? As fight as light !{bcolors.ENDC}")
        elif 5 <= answer_time < 10:
            print(f"{bcolors.WARNING}{answer_time}{bcolors.OKBLUE} seconds to answer... Not bad at all !{bcolors.ENDC}")
        else:
            print(f"{bcolors.FAIL}{answer_time}{bcolors.OKBLUE} seconds to answer... That's to slow !{bcolors.ENDC}")

        # Sleeping half a second
        sleep(0.5)

    # Printing good answers number
    print(f"{bcolors.OKBLUE}You got {bcolors.OKGREEN}{correct_answers}{bcolors.OKBLUE} correct answers.{bcolors.ENDC}")

    if len(wrong_answers) == 0:  # If there are no wrong answers
        # Printing a little congrats message
        print(f"{bcolors.OKGREEN}And no wrong one ! Congratulations !{bcolors.ENDC}")
        ...
        print(f"{bcolors.FAIL}You missed for :\n- {wrong_answers}{bcolors.ENDC}")
elif mode in (2, 3, 6, 7):
    # Mode 2 and 3 are identical, except the fact mode 3 or 7 shuffles the verbs order
    if mode == 6 or mode == 7:
        print(f"{bcolors.WARNING}Which lines do you want to work on ?{bcolors.ENDC}")
        min_line = int(input(f"{bcolors.WARNING}Minimum line : {bcolors.ENDC}")) - 1
        max_line = int(input(f"{bcolors.WARNING}Maximum line : {bcolors.ENDC}"))
    ...
    for i in range(len(verbs_list)):
        ...
        for k in range(len(answer)):
            ...
            if isinstance(verbs_list[i][k], list):
                if multi_answers_require_all is True:
                    ...
                    if answer[k].lower() in verbs_list[i][k]:  # If it is correct
                        answer[k] = f"{column_order[k][0].upper()+column_order[k][1:]} : {bcolors.HEADER}{answer[k]}{bcolors.OKGREEN} -> Correct !{bcolors.ENDC}"
                    else:  # Otherwise
                        answer[k] = f"{column_order[k][0].upper()+column_order[k][1:]} : {bcolors.HEADER}{answer[k]}{bcolors.FAIL} -> Wrong ! Answer : '{bcolors.HEADER}{recreate_string(verbs_list[i][k], '; ')[:-2]}{bcolors.FAIL}'{bcolors.ENDC}"
            if do_not_the_other_statement is False:
                if answer[k].lower() == verbs_list[i][k]:  # If it is correct
                    ...
                    if isinstance(verbs_list[i][k], list):
                        verbs_list[i][k] = recreate_string(verbs_list[i][k], "; ")[:-2]
                    answer[k] = f"{column_order[k][0].upper()+column_order[k][1:]} : {bcolors.HEADER}{answer[k]}{bcolors.FAIL} -> Wrong ! Answer : '{bcolors.HEADER}{verbs_list[i][k]}{bcolors.FAIL}'{bcolors.ENDC}"
        ...
        if answer_time < 5:
            print(f"{bcolors.OKGREEN}{answer_time}{bcolors.OKBLUE} seconds max to answer ? As fast as the light !{bcolors.ENDC}")
        elif 5 <= answer_time < 10:
            print(f"{bcolors.WARNING}{answer_time}{bcolors.OKBLUE} seconds max to answer... Not bad at all !{bcolors.ENDC}")
        else:
            print(f"{bcolors.FAIL}{answer_time}{bcolors.OKBLUE} seconds max to answer... That's to slow !{bcolors.ENDC}")
    ...
    while running:
        ...
        if loop_times % 5 == 0:
            confirm = input(f"{bcolors.OKBLUE}Do we stop or do we keep going ? ('quit' or 'stop' to leave the program, otherwise it continues !) : {bcolors.ENDC}")
            if confirm.lower() == "quit" or confirm.lower() == "stop":
                running = False
            del confirm
